...
CREATE TABLE em_tabela (
  clube_nome      VARCHAR2(30)    CONSTRAINT nn_nome_em             NOT NULL,
  epoca_ano       NUMBER(4)       CONSTRAINT nn_ano_em              NOT NULL,
  nif_jogadora    NUMBER(6)       CONSTRAINT nn_nif_jogadora_em     NOT NULL,
  nif_treinador   NUMBER(6),
...
CREATE TABLE pessoa (
  ...
  -- IMPORTAR ANO EPOCA
  epoca_ano      NUMBER(4)      CONSTRAINT nn_ano_pessoa       NOT NULL,
...
CREATE TABLE treinador (
  ...
  --data_nasc_treinador   DATE          CONSTRAINT nn_data_nasc_treinador NOT NULL,
...
--    
  CONSTRAINT fk_nif_treinador
    ...
    REFERENCES pessoa(nif)
--  
 -- CONSTRAINT fk_data_nasc_treinador
 --  FOREIGN KEY (data_nasc_treinador)
 --  REFERENCES pessoa(data_nascimento),
--
...
CREATE TABLE participa (
  ...
  nome_clube_visitante      VARCHAR2(30)   CONSTRAINT nn_nome_club_visitante_part       NOT NULL,
  nome_clube_casa           VARCHAR2(30)   CONSTRAINT nn_nome_club_casa_part            NOT NULL,
  dia                       DATE           CONSTRAINT nn_dia_participa                  NOT NULL,
  
  CONSTRAINT ck_minutos_part
    CHECK (minuto_entrada <> NULL AND minuto_saida <> NULL AND minuto_entrada < minuto_saida),
    
 CONSTRAINT fk_nome_clube_visitante_part
   FOREIGN KEY (nome_clube_visitante)
   REFERENCES jogo(nome_clube_visitante),
   
    CONSTRAINT fk_nome_clube_casa_part
   FOREIGN KEY (nome_clube_casa)
   REFERENCES jogo(nome_clube_casa),
